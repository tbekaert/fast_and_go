 ...
 * });
 */
let MoveElements = class MoveElements {
  constructor(opts) {
    ...
    window.addEventListener(
      ...
      (function(_this) {
        return function() {
          ...
          return (_this.resizeTimer = setTimeout(
            _this.handler.bind(_this),
            50
          ));
        };
      })(this),
  ...
  move(bp) {
    if (bp.insert) {
      if (bp.insert === "before") {
        ...
        bp.element.parentNode.insertBefore(
          this.element,
          bp.element.nextElementSibling
        );
      }
    } else if (bp.append) {
      if (bp.append === "before") {
