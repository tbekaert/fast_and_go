...
(function (selector) {
  Array.from(document.querySelectorAll(selector)).forEach(function (link) {
    link.addEventListener('click', function (e) {
      e.preventDefault();
      var anchor = document.querySelector(link.getAttribute('data-target'));
      window.scrollBy({
...
(function (_ref) {
  var $menuTriggers = _ref.$menuTriggers,
      $nav = _ref.$nav,
      $header = _ref.$header,
      $links = _ref.$links;
  ...
  var links = $links.map(function ($link) {
    ...
    var targetTop = targetElement.getBoundingClientRect().top + window.scrollY - window.innerHeight * 0.25;
  ...
  var getActiveSectionIndex = function getActiveSectionIndex() {
  ...
  var onScroll = function onScroll(activeLinkIndex) {
    var currentActiveSection = parseInt(getActiveSectionIndex(), 10);
  ...
  var updateActiveSection = function updateActiveSection() {
    var link = links[activeSectionIndex];
    links.map(function (l) {
      return l.element.classList.remove('active');
    });
    link.element.classList.add('active');
  };
  window.addEventListener('scroll', onScroll);

  $menuTriggers.map(function ($trigger) {
    return $trigger.addEventListener('click', function (e) {
      return $nav.classList.toggle('show');
    });
  });
  var headerScrolling = basicScroll.create({
    elem: $header,
    from: 0,
...
(function (_ref) {
  var loss_rate = _ref.loss_rate,
      machine_price = _ref.machine_price,
      elements = _ref.elements;

  var $ = elements.reduce(function (r, s) {
  ...
  var profitability_text = $.profitability.getAttribute('data-text');

  var handleClick = function handleClick(e) {
    e.preventDefault();
    if (shouldShowResults) {
      var loss_by_keg = loss_rate * values.keg_volume;
      var keg_price = values.keg_volume / (values.glasses_volume / 100) * values.avg_price;
      var liter_price = keg_price / values.keg_volume;
      var lost_money_by_keg = loss_by_keg * liter_price;
      var monthly_loss = lost_money_by_keg * values.keg_number;

      var profitable = Math.floor(machine_price / monthly_loss) + 1;
  ...
  ['keg_volume', 'keg_number', 'glasses_volume', 'avg_price'].map(function (name) {
    $[name].addEventListener($[name].tagName === 'INPUT' ? 'keyup' : 'change', function (e) {
      values[name] = Number(e.target.value.replace(',', '.'));
      if (Object.keys(values).every(function (k) {
        return values[k];
      })) {
  ...
  loss_rate: 0.2,
  machine_price: 2490,
  elements: ['keg_volume', 'keg_number', 'glasses_volume', 'avg_price', 'simulation_toggle', 'view_1m', 'view_1y', 'view_3y', 'view_5y', 'view_10y', 'profitability', 'simulation_container']
});
...
(function (_ref) {
  var elements = _ref.elements,
      closingButtons = _ref.closingButtons;
  ...
  var links = elements.map(function (element) {
  ...
  var parents = links.reduce(function (t, e) {
    return [].concat(toConsumableArray(t), [e.parent]);
  }, []);
  console.log(parents, elements);
  links.map(function (link) {
    link.element.addEventListener('click', function (e) {
      e.preventDefault();
      activateSection(link);
    });
  });
  closingButtons.map(function (button) {
    button.addEventListener('click', function (e) {
      e.preventDefault();
      [].concat(toConsumableArray(parents), toConsumableArray(links.reduce(function (t, e) {
        return [].concat(toConsumableArray(t), [e.element, e.target]);
      }, []))).map(deactivate);
    });
  });
  var activateSection = function activateSection(_ref2) {
    var element = _ref2.element,
        target = _ref2.target,
        parent = _ref2.parent;

    var isAlreadyActive = element.classList.contains('active');
    [].concat(toConsumableArray(parents), toConsumableArray(links.reduce(function (t, e) {
      return [].concat(toConsumableArray(t), [e.element, e.target]);
    }, []))).map(deactivate);
